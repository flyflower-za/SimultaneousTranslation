...
- 📚 控制端“我的会议”历史、Markdown/PDF 下载；管理端纪要版本、模型成本及失败重试管理
- 🎨 现代化三列布局界面
...
模型未配置时，实时翻译继续工作，会议和字幕仍会保存；管理员可在“纪要模型”页配置模型后重新生成，或直接手工编辑草稿。
纪要默认只在管理员发布后向持有查看端分享链接的人开放。
管理员也可在管理后台的“纪要模型”页修改地址、模型、密钥、重试次数和 Token 单价，保存后立即生效；
这些设置写入本地 `data/access.db`，API Key 不会回传到浏览器，环境变量仍优先。
控制端可打开 `/my-meetings` 查看自己的会议历史、逐字稿和纪要，并下载 Markdown 或 PDF。
管理端的每次手工保存、模型生成及旧版恢复都会形成新版本；发布会固定到当时的版本。
模型成本根据接口返回的 Token 用量和管理端配置的单价估算，接口未提供用量时显示“未知”。
